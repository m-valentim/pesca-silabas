...
func construir_cenario_e_ui():
	...
	var container_vidas = HBoxContainer.new(); container_vidas.position = Vector2(100, 20); add_child(container_vidas)
	for i in range(3):
		var coracao = Label.new(); coracao.text = "♥"; coracao.add_theme_font_size_override("font_size", 50); coracao.add_theme_color_override("font_color", Color(1, 0.2, 0.2)) 
		container_vidas.add_child(coracao); coracoes.append(coracao)
